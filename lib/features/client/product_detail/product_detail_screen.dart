...
class _ProductDetailScreenState extends ConsumerState<ProductDetailScreen> {
  ...
  double _totalFor(ProductModel product) {
    final addTotal = product.additionals.fold<double>(
      0.0,
      (sum, a) => sum + a.price * (_addQty[a.id] ?? 0),
    );
    return (product.price + addTotal) * _qty;
  }

  void _addToCart(ProductModel product, String storeName) {
    final selected = product.additionals
        .where((a) => (_addQty[a.id] ?? 0) > 0)
        .map((a) => SelectedAdditional(additional: a, qty: _addQty[a.id]!))
        .toList();
    ...
    return Scaffold(
      ...
      body: detailAsync.when(
        data: (detail) {
          ...
          final storeName = detail.store?.name ?? 'Cantina';
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('Caracterizado por $storeName',
                                          style: const TextStyle(
                                              fontSize: 12,
                                          ...
                                          children: [
                                            Text(add.name,
                                                style: const TextStyle(
                                                    ...
                                                    fontWeight: FontWeight.w600,
                                                    color: AppColors.textDark)),
                                            Text('+ ${_currency.format(add.price)}',
                                                style: const TextStyle(
                                                    fontSize: 12,
                    ...
                    Expanded(
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () => _addToCart(product, storeName),
                          ...
                          child: Text(
                            'ADICIONAR ${_currency.format(_totalFor(product))}',
                            style: const TextStyle(
                                fontWeight: FontWeight.w700,
                                fontSize: 14,
                                color: Colors.white),
        ...
        error: (_, __) => const Center(child: Text('Erro ao carregar produto')),
      ),
    );
  }
}
